...
def predictions(weather_turnstile):
    ...
    features = weather_turnstile[['rain', 'Hour', 'mintempi', 'maxtempi', 'meanwindspdi','fog','precipi']].join(dummy_units)
    values = weather_turnstile[['ENTRIESn_hourly']]
    m = len(values)

    """
    Linear Regression with statsmodels:
    R^2 value is: 0.485414650479
    """
    model = sm.OLS(values, features)
    results = model.fit()
    theta = results.params
    prediction = results.fittedvalues
    #print theta, results.rsquared
